...
abstract class TweetSet {
  ...
  def findMaxVal(maxValue:Tweet):Tweet
  /**
...
class Empty extends TweetSet {
  ...
  def mostRetweeted:Tweet=throw new NoSuchElementException("there is no mostRetweeted in empty list")
  def findMaxVal(maxVal:Tweet):Tweet=maxVal
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  {
    if(elem.retweets>maxVal.retweets) left.findMaxVal(right.findMaxVal(elem))
    else left.findMaxVal(right.findMaxVal(maxVal))
  }
